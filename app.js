...
function guardarItem(event) {
    ...
    const idExistente = document.getElementById('item-id').value;
    ...
    return `
        <div class="cajero-item ${adminClass}" id="cajero-${cajero.id}">
            <div class="cajero-info">
                ...
                ${usuarioEsAdmin ? `<p style="margin:2px 0;font-size:0.85rem;color:#94a3b8;">🔑 ${passwordMask}</p>` : ''}
            </div>
...
function cargarInventarioGeneral(filtro = 'todo') {
    ...
    tbody.innerHTML = todos.map(item => {
        ...
        return `
            <tr>
                <td>
                    <span class="badge" style="background: #dbeafe; color: #1e40af; display: flex; align-items: center; gap: 6px;">
                ...
                <!-- NUEVA COLUMNA: ID -->
                <td>
                    <code style="background: #f1f5f9; padding: 4px 8px; border-radius: 4px; font-size: 0.85rem; color: #64748b;">
                        ${item.id}
                    </code>
                </td>
